...
int MD2Model::load(char* model_file, char* skin_file) {
    ...
    // 确认文件长度
    is.seekg(0, is.end);
    long long int file_length = is.tellg();
    is.seekg(0, is.beg);
    ...
    for (int frame_index = 0; frame_index < num_frames; ++frame_index) {
        frame_ptr = (Frame*)&file_buffer[model_header->offset_frames + frame_size * frame_index];
        
        vertex_ptr = (Vertex3f*)&vertex_list[num_vertices * frame_index];
        for (int vertex_index = 0; vertex_index < num_vertices; ++ vertex_index) {
            vertex_ptr[vertex_index].v[0] = frame_ptr->scale[0] * frame_ptr->fp[vertex_index].v[0] + frame_ptr->translate[0];
            vertex_ptr[vertex_index].v[1] = frame_ptr->scale[1] * frame_ptr->fp[vertex_index].v[1] + frame_ptr->translate[1];
            vertex_ptr[vertex_index].v[2] = frame_ptr->scale[2] * frame_ptr->fp[vertex_index].v[2] + frame_ptr->translate[2];
        }
    }
    ...
    TexCoord2s* tex_coord_2s_ptr = (TexCoord2s*)(&file_buffer[model_header->offset_tex_coord]);
